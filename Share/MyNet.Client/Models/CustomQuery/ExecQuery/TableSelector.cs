...
namespace MyNet.Client.Models.CustomQuery.ExecQuery
{
    public class TableSelector
    {
        ...
        private void AddJoinTableAction(object obj)
        {
            ...
            {
                MessageWindow.ShowMsg(MessageType.Info, OperationDesc.Cq_AddJoinTable, "请选择主表");
                return;
            }
        ...
        private void DelJoinTableAction(object obj)
        {
            ...
            var rst = MessageWindow.ShowMsg(MessageType.Ask, OperationDesc.Cq_DelJoinTable, "删除关联表将同时删除对应关联字段，是否继续？");
            if (rst == true)
            {
        ...
        private void AddRelFieldAction(object obj)
        {
            ...
            if (table == null)
            {
                MessageWindow.ShowMsg(MessageType.Info, OperationDesc.Cq_AddRelField, "请先添加关联表");
                return;
            }
            if (table.Table.IsEmpty())
            {
                MessageWindow.ShowMsg(MessageType.Info, OperationDesc.Cq_AddRelField, "请先选择关联表");
                return;
            }
