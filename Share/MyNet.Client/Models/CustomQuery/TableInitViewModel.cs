...
namespace MyNet.Client.Models.CustomQuery
{
    public class TableInitViewModel : MngViewModel
    {
        ...
        private void SaveAction(object obj)
        {
            ...
            //
            var askRst = MessageWindow.ShowMsg(MessageType.Ask, OperationDesc.Cq_InitTables, "初始化操作将清空当前查询表以及查询字段，是否继续？");
            if (askRst != true)
            {
                return;
            }
            var rst = HttpUtils.PostResult(ApiUtils.GetApiUrl(ApiKeys.Cq_TableInit), sels, ClientContext.Token);
            if (rst.code != ResultCode.Success)
            {
                MessageWindow.ShowMsg(MessageType.Error, OperationDesc.Cq_InitTables, rst.msg);
                return;
            }
            MessageWindow.ShowMsg(MessageType.Info, OperationDesc.Cq_InitTables, MsgConst.Msg_Succeed);
            if (Window != null)
            {
