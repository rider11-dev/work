...
namespace MyNet.Components.Http
{
    public class HttpUtils
    {
        static ILogHelper<HttpUtils> _logHelper = LogHelperFactory.GetLogHelper<HttpUtils>();
        ...
        public static string Get(string url, string token = "")
        {
            ...
            try
            {
                ...
                using (var stream = response.GetResponseStream())
                {
                    ...
                    //记录本次请求信息
                    _logHelper.LogInfo(string.Format("request:{0}\turl,{1}{0}response:{0}\t{2}",
        ...
        public static OptResult GetResult(string url, string token = "")
        {
            var str = Get(url, token);
            var rst = JsonConvert.DeserializeObject<OptResult>(str);
            return rst;
        }
        ...
        public static string Post(string url, object jsonData, string token = "")
        {
            ...
            try
            {
                ...
                using (var stream = response.GetResponseStream())
                {
                    ...
                    var strResponse = reqEncoding.GetString(bytes, 0, bytes.Length);
                    //记录本次请求信息
                    _logHelper.LogInfo(string.Format("request:{2}\turl,{0}{2}\tjsonData,{1}{2}response:{2}\t{3}",
        ...
        public static OptResult PostResult(string url, object jsonData, string token = "")
        {
            ...
            Action<AsyncCompletedEventArgs> downloadCompleteCallback = null)
        {
            ...
            try
            {
                ...
                client.DownloadFileCompleted += (o, e) =>
                {
                    if (e.Error == null)
                    {
                        _logHelper.LogInfo(string.Format("文件下载成功，from\t{0}\tto\t{1}", url, filename));
                    }
                    else
                    {
                        _logHelper.LogError(string.Format("文件下载失败，from\t{0}\tto\t{1}", url, filename), e.Error);
                    }
