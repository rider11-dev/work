...
namespace  AllInOne.Client
{
    public class CardQueryViewModel : BaseModel
    {
        ...
        private void QueryAction(object obj)
        {
            ...
            //1、查询账户信息
            var rst = HttpUtils.PostResult(ApiUtils.GetApiUrl(AIOApiKeys.GetCardAccount, AIOApiKeys.Key_ApiProvider_Card), new { idcard = idcard });
            ...
            //2、查询卡信息
            rst = HttpUtils.PostResult(ApiUtils.GetApiUrl(AIOApiKeys.GetCards, AIOApiKeys.Key_ApiProvider_Card), new { idcard = idcard });
            if (rst.code != ResultCode.Success)
            {
                MessageWindow.ShowMsg(MessageType.Error, OperationDesc.Search, rst.msg);
                return;
